                     ...
                     int protocol)
{
    ...
    next = results;
    while (next) {
        /* 未进行 service 解析，进行地址去重  */
        if (*port >= 0x30 && *port <= 0x39) {
            if (next->ai_addr->sa_family == AF_INET && next->ai_protocol == protocol)
                result.push_back(IPv4Address::ptr(new IPv4Address(*(sockaddr_in*)next->ai_addr)));
            else if (next->ai_addr->sa_family == AF_INET6 && next->ai_protocol == protocol)
                result.push_back(IPv6Address::ptr(new IPv6Address(*(sockaddr_in6*)next->ai_addr)));
        } else {
...
{
    m_len = offsetof(sockaddr_un, sun_path) + strlen(m_addr.sun_path);
}
...
{
    return (uint32_t)ntohs(m_addr.sin6_port);
}
