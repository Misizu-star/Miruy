    ...
    : m_family(family), m_type(type), m_protocol(protocol)
{
    ...
    if (m_sock != -1)
        ...
        throw std::logic_error("create socket error");
    }
}

void Socket::init(int sock, bool is_stream)
{
    Fd::ptr fd_ctx = FdMgr::getInstance()->getfd(sock);
...
{
    Fd::ptr fd_ctx = FdMgr::getInstance()->getfd(m_sock);
...
{
    auto fd_ctx = FdMgr::getInstance()->getfd(m_sock);
    if (fd_ctx)
        fd_ctx->setTimeout(SO_SNDTIMEO, timeout_ms);
    struct timeval tv {
...
bool Socket::setRecvTimeout(uint64_t timeout_ms)
{
    auto fd_ctx = FdMgr::getInstance()->getfd(m_sock);
    if (fd_ctx)
        fd_ctx->setTimeout(SO_RCVTIMEO, timeout_ms);
    struct timeval tv {
        time_t(timeout_ms / 1000), suseconds_t(timeout_ms % 1000000)
    };
    return setOption(SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv));
...
void Socket::setNonBlock(bool flag) const
{
    Fd::ptr fd_ctx = FdMgr::getInstance()->getfd(m_sock);
    fd_ctx->SetNonBlock(m_sock, flag);
}
...
int Socket::send(const void* buff, size_t len, int flag) const
{
    if (!isConnected())
        return -1;
...
int Socket::send(const iovec* buffs, size_t len, int flag) const
{
    if (!isConnected())
        return -1;
    msghdr msg;
...
int Socket::sendTo(const void* buff, size_t len, const Address::ptr to, int flag) const
{
    return isConnected() ? ::sendto(m_sock, buff, len, flag, to->getAddr(), to->getAddrLen()) : -1;
}

int Socket::sendTo(const iovec* buffs, size_t len, const Address::ptr to, int flag) const
{
    if (!isConnected())
        return -1;
    msghdr msg;
...
int Socket::recv(void* buff, size_t len, int flag) const
{
    return isConnected() ? ::recv(m_sock, buff, len, flag) : -1;
}

int Socket::recv(iovec* buffs, size_t len, int flag) const
{
    if (!isConnected())
        return -1;
    msghdr msg;
...
int Socket::recvFrom(void* buff, size_t len, Address::ptr from, int flag) const
{
    if (!isConnected())
        return -1;
...
int Socket::recvFrom(iovec* buffs, size_t len, Address::ptr from, int flag) const
{
    if (!isConnected())
        return -1;
    msghdr msg;
...
{
    std::stringstream ss;
    ss << std::boolalpha << "[Socket sock=" << m_sock << " family=" << m_family
       << " type=" << m_type;
    if (m_local)
