...
namespace miruy {
    ...
    Semaphore::Semaphore(uint32_t count)
    {
        if (sem_init(&m_sem, 0, count)) {
            LOG_ERROR(LOGGER("system"), "Semaphore::Semaphore() sem_init error");
    ...
    {
        if (sem_wait(&m_sem)) {
            LOG_ERROR(LOGGER("system"), "Semaphore::wait() sem_wait error");
    ...
    {
        if (sem_post(&m_sem)) {
            LOG_ERROR(LOGGER("system"), "Semaphore::notify() sem_post error");
    ...
    Mutex::Mutex() { pthread_mutex_init(&m_lock, nullptr); }
    Mutex::~Mutex() { pthread_mutex_destroy(&m_lock); }
    void Mutex::lock() { pthread_mutex_lock(&m_lock); }
    void Mutex::unlock() { pthread_mutex_unlock(&m_lock); }
    ...
    void CASLock::unlock() { std::atomic_flag_clear_explicit(&m_mutex, std::memory_order_release); }
